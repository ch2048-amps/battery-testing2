...
try:
    ...
    for i in times:
        ...
        Vout = meter.query('MEAS:VOLT:DC?') # get the current voltage
        print(Vout)                       
        temperature[time_idx] = (float(Vout) * 25 / 2.98) # temp of the system
        time.sleep(0.25)
        time_idx = time_idx + 1
